...
def setup_experiment(args):
    ...
    experiment_name = args.experiment_name if args.experiment_name else utils.get_current_time()
...
def setup_nn_and_optimiser(args, microgrid):
...
def setup_losses():
    ...
    return regression_loss, total_cost_loss


def setup_trainer(args, experiment_path):
    tb_logger = pl.loggers.TensorBoardLogger(save_dir=experiment_path, name="logs") if args.st_checkpoint_dir is None else None
...
def main(args):
    ...
    microgrid = setup_plant_model(args)
    controller = setup_nn_and_optimiser(args, microgrid)
    regression_loss, total_cost_loss = setup_losses()
    trainer = setup_trainer(args, experiment_path)
    ...
    results = {}
    data_splits = ['train', 'val', 'test'] if args.plot else ['test']
